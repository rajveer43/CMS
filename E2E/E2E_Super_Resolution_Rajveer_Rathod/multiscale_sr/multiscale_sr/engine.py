...
def evaluate(
    ...
    ``semd`` is likewise computed unconditionally — including in runs that do
    not optimize it (``--lambda-semd 0.0``) — so that correlation data against
    tagging efficiency accumulates from every run rather than only from
    SEMD-enabled ones.
    """
    generator.eval()
    l1_sum = 0.0
